...
<?php 
  ...
  $totdeduct = $tax + $row['advances'] +  $row['sss']+$row['phealth']+$row['pibig']; 
...
if (isset($_POST['insert'])) {
  ...
  if ($insert) {
    $query = "INSERT INTO doctors VALUES ($empno,'$lname','$fname','$init','$position',$pay,$dayswork,$otrate,$othrs,$allow,$advances,$insurance)";
	$msg = "New record saved!";
  }
  else {
    $query = "UPDATE doctors SET empno=$empno,lname='$lname',fname='$fname',init= '$init',position='$position' WHERE id = $id";
    $msg = "Record updated!";
  }
  include 'include/dbconnection.php';
...
if (isset($_POST['insert'])) {
  ...
  $advances = $_POST['advances'];
   $sss = $_POST['sss'];
  $pbig = $_POST['p-ibig'];
  $phealth = $_POST['p-health'];
  ?>
  
<?php
  if ($insert) {
	  
    $query = "INSERT INTO doctors VALUES ($id,$empno,',$pay,$dayswork,$otrate,$othrs,$allow,$advances,$sss,&p-ibig,&p-health)";
	
$msg ="<center><table border='1' width='431'  ><td bgcolor='#009933'><center>New record saved!</center></label></table></center>";
  }
  else {
    $query = "UPDATE doctors SET id=$id,empno=$empno,',pay=$pay,dayswork=$dayswork,otrate=$otrate,othrs=$othrs,allow=$allow,advances=$advances,insurance=$insurance WHERE empno = $empno";
...
<table>
	 <tr>
            ...
            <td align="right"><?=$dayswork?></td>
          </tr>
      <tr>
            <td class="box" align="right">OT Hours:</td>
            <td align="left"><?=$othrs?></td>
          </tr>
</table>
...
<tr>
  ...
  <tr>
    <td class="box">SSS</td>
    <td align="right"><?php echo "$sss"; ?></td>
  </tr>
    <tr>
    <td class="box">Pag-Ibig</td>
    <td align="right"><?php echo "$pibig"; ?></td>
  </tr>
  <tr>
    <td class="box">Philhealth</td>
    <td align="right"><?php echo "$phealth"; ?></td>
  </tr>
   </tr>
